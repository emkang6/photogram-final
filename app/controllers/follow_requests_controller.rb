class FollowRequestsController < ApplicationController
  ...
  def create
    ...
    @corr_recipient = User.find_by(id: @recipient_id)
    @recipient_name = @corr_recipient.username


    if the_follow_request.valid?
      the_follow_request.save
      redirect_to("/users/" + @recipient_name, { :notice => "Follow request created successfully." })
    else
